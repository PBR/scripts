...
v=c(v,rep("Group_5_10_samples",10))
#v=c(v,"Eriopersicon")
...
for (i in 1:nrow(CNVs_mean_84_count)) 
    {
        ...
        Group_5_10_samples_v=unname(unlist(table_with_CHR_Gene[,c("X78","X19","X83","X44","X73","X18","X28","X35","X37","X75")]))
        Group_5_10_samples_v_var=var(as.numeric(Group_5_10_samples_v))
        Group_5_10_samples_v_mean=mean(as.numeric(Group_5_10_samples_v))
        merged_v=as.numeric(c(Lycopersicum_v,Group_5_10_samples_v))
        #print(Lycopersicum_v_mean)
        #print(Group_2_9_samples_v_mean)
        #print(merged_v)
        dif_var=abs(Lycopersicum_v_var - Group_5_10_samples_v_var)
        dif_mean=(Lycopersicum_v_mean - Group_5_10_samples_v_mean)
        #print(dif_mean)
        ...
        p.val_Group_5_10_samples=coef_table[2,4]
        #print (p.val_Group_2_9_samples)
        F.pvalue=p
        t.pval=p.val_Group_5_10_samples
        if (is.na(t.pval))
            {
                t.pval=1
                F.pvalue=1
        ...
        results=data.frame(CHR,gene,F.pvalue,t.pval,Lycopersicum_v_var,Group_5_10_samples_v_var,dif_var,Lycopersicum_v_mean,Group_5_10_samples_v_mean,dif_mean)
        write.table(results,"CNVs_mean_84_count_lm_Lyc_1_VS_Gr_5-10-samples.txt",append=T,col.names=T, row.names=T)
         #   }
