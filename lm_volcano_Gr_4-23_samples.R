...
v=c(v,rep("Group_4_23_samples",23))
#v=c(v,"Eriopersicon")
...
for (i in 1:nrow(CNVs_mean_84_count)) 
    {
        ...
        Group_4_23_samples_v=unname(unlist(table_with_CHR_Gene[,c("X60","X59","X46","X56","X23","X58","X68","X57","X70","X54","X63","X66","X67","X61","X62","X64","X65","X69","X53","X52","X55","X47","X48")]))
        Group_4_23_samples_v_var=var(as.numeric(Group_4_23_samples_v))
        Group_4_23_samples_v_mean=mean(as.numeric(Group_4_23_samples_v))
        merged_v=as.numeric(c(Lycopersicum_v,Group_4_23_samples_v))
        #print(Lycopersicum_v_mean)
        #print(Group_2_9_samples_v_mean)
        #print(merged_v)
        dif_var=Mod(Lycopersicum_v_var - Group_4_23_samples_v_var)
        dif_mean=(Lycopersicum_v_mean - Group_4_23_samples_v_mean)
        #print(dif_mean)
        ...
        p.val_Group_4_23_samples=coef_table[2,4]
        #print (p.val_Group_2_9_samples)
        F.pvalue=p
        t.pval=p.val_Group_4_23_samples
        if (is.na(t.pval))
            {
                t.pval=1
                F.pvalue=1
        ...
        results=data.frame(CHR,gene,F.pvalue,t.pval,Lycopersicum_v_var,Group_4_23_samples_v_var,dif_var,Lycopersicum_v_mean,Group_4_23_samples_v_mean,dif_mean)
        write.table(results,"CNVs_mean_84_count_lm_Lyc_1_VS_Gr_4-23-samples.txt",append=T,col.names=T, row.names=T)
         #   }
