...
variance <- read.table("/media/bulk_01/users/goryu003/samples/Variance-3.txt",as.is=TRUE,header=T)
head(variance)
head(variance$Lycopersicum_v_var)
pdf( "density.pdf", width =10 , height =10 )
plot(density(variance$Lycopersicum_v_var), col = 'blue')
lines(density(variance$Group_2_9_samples_v_var), col = 'red')
dev.off()
