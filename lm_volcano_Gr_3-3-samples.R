...
for (i in 1:nrow(CNVs_mean_84_count)) 
    {
        ...
        Group_3_3_samples_v=unname(unlist(table_with_CHR_Gene[,c("X13","X10","X7")]))
        Group_3_3_samples_v_var=var(as.numeric(Group_3_3_samples_v))
        Group_3_3_samples_v_mean=mean(as.numeric(Group_3_3_samples_v))
        merged_v=as.numeric(c(Lycopersicum_v,Group_3_3_samples_v))
        #print(Lycopersicum_v_mean)
        #print(Group_2_9_samples_v_mean)
        #print(merged_v)
        dif_var=abs(Lycopersicum_v_var - Group_3_3_samples_v_var)
        dif_mean=(Lycopersicum_v_mean - Group_3_3_samples_v_mean)
        #print(dif_mean)
        ...
        p.val_Group_3_3_samples=coef_table[2,4]
        #print (p.val_Group_2_9_samples)
        F.pvalue=p
        t.pval=p.val_Group_3_3_samples
        if (is.na(t.pval))
            {
                t.pval=1
                F.pvalue=1
        ...
        results=data.frame(CHR,gene,F.pvalue,t.pval,Lycopersicum_v_var,Group_3_3_samples_v_var,dif_var,Lycopersicum_v_mean,Group_3_3_samples_v_mean,dif_mean)
        write.table(results,"CNVs_mean_84_count_lm_Lyc_1_VS_Gr_3-3-samples.txt",append=T,col.names=T, row.names=T)
         #   }
